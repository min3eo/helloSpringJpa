 ...
 * =====================================================================
 *
 * Repository 역할: 데이터 저장소(DB)에 대한 접근 로직을 캡슐화합니다.
 * Service 계층은 데이터가 어디에 있는지(DB, 캐시, 파일 등) 알 필요 없이
 * Repository 인터페이스만 사용합니다.
 *
 * @Repository : 다음 두 가지 역할을 합니다.
 *   1. @Component의 특수화로 Spring이 해당 클래스를 빈으로 등록합니다.
 *   2. DataAccessException 변환기 활성화
 *      JDBC/JPA 예외를 Spring의 DataAccessException 계층으로 변환합니다.
 *      상위층(서비스 계층)은 특정 DB 기술의 예외에 의존하지 않아도 됩니다.
 *
 * [EntityManager 소개]
 ...
 * 트랜잭션 종료 시 변경된 엔티티를 자동으로 DB에 반영합니다.(엔티티 변경 감지)
 */
@Repository
public class ProductRepository {
     ...
     * 트랜잭션 범위의 프록시 객체입니다.
     * 즉 실제 EntityManager는 현재 트랜잭션에 바인딩된 것을 사용합니다.
     * 따라서 싱글톤 서비스/리포지토리 환경에서도 안전하게 사용 가능합니다.
     */
    @PersistenceContext
     ...
     *   - "FROM Product p" 는 product 테이블이 아닌 Product 엔티티를 조회
     *   - Hibernate가 실제 SQL로 변환하여 실행합니다.
     *
     * TypedQuery<Product>: 타입 안전한 쿼리 (ClassCastException 방지)
     */
    public List<Product> findAll() {
        // LEFT JOIN FETCH: LAZY인 category를 한 번의 쿼리로 즉시 로드 (LazyInitializationException 방지)
     ...
     *   - 주의: Detached 엔티티는 먼저 merge()로 Managed 상태로 변경 필요
     */
