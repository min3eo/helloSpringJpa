 ...
 *   - Spring이 해당 클래스를 빈으로 등록합니다.
 *   - 비즈니스 로직 담당 클래스임을 의미적으로 명확하게 표현합니다.
 *
 ...
 *   3. 단위 테스트 시 Mock 객체 주입이 쉽습니다.
 *   4. 순환 의존성을 컴파일 타임에 감지할 수 있습니다.
 *
 * [@Transactional 상세 설명]
 * 클래스 선언부의 어노테이션은 모든 public 메서드에 트랜잭션을 적용합니다.
 *
 * 트랜잭션 전파(Propagation):
 ...
 *
 * readOnly = true:
 *   - 읽기 전용 최적화: Hibernate의 엔티티 변경감지(Dirty Checking)를 비활성화합니다.
 *   - DB 드라이버 레벨에서 읽기 전용 연결로 설정될 수 있어 성능이 향상됩니다.
 *   - 읽기 메서드에는 반드시 readOnly = true를 명시하는 것을 권장합니다.
 */
@Service
@Transactional(readOnly = true) // 클래스 기본값은 읽기 전용 트랜잭션
public class ProductService {
    ...
    /**
     * 모든 상품 조회
     * readOnly = true (클래스 선언부 기본 설정 상속): 읽기 전용 트랜잭션
     */
     ...
     * 화면(컨트롤러, Validation)과 서비스 레이어 양쪽에서 검증하는 이유:
     *   - 방어적 프로그래밍(Defense in Depth): 웹 화면을 우회하는 API 호출이나
     *     테스트 코드에서도 비즈니스 규칙을 항상 보장합니다.
     *   - 서비스는 "어떤 클라이언트가 호출해도" 유효성을 보장해야 합니다.
     */
    @Transactional
